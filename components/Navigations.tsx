...
const Navigations: React.FC<INav> = ({ isModal }) => {
  ...
  return (
    <nav  className={`fixed text-white bottom-10 left-4 md:left-16 ${isModal ? 'z-1' : 'z-30'}`}>
      <div>
        <Link 
          href="/" 
        ...
        <div>
          <Link 
          href="/techs" 
          className={`block text-textColor text-[1.2rem] py-2 hover:px-2 light-up hover:pl-2 transition-all duration-500 ${pathname === "/techs" ? 'inner-shadow pl-2' : '' }`}
          >Techs</Link>
          <div className="h-[2px] w-[5.5rem] bg-textColor"></div>
        </div>
        <div>
